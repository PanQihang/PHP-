...
else
{
    ...
    foreach ($pdo->query($sql) as $row)
    {
        if($row['Audit']==null || $row['Audit']==0 || $row['Audit']==2)
        {
            ...
            echo "<td width='100' class='color-text'><button id='{$row['WFlowID']}' name='{$row['WFlowID']}' onclick='check1({$row['WFlowID']})' value='{$row['WFlowID']}' class='btn btn-block'><i style='color: green' class=\"fa fa-check\"></i></button></td>";
            echo "<td width='100' class='color-text'><button id='{$row['WFlowID']}no' name='{$row['WFlowID']}no' onclick='check2({$row['WFlowID']})' value='{$row['WFlowID']}' class='btn btn-block'><i style='color: red' class=\"fa fa-close\"></i></button></td>";
            echo "</tr>";
        }
    }
    if($flag==0)
